...
## Install The Nexis Network CLI Locally

To create your validator vote account, you need to install the
[Nexis Network command line interface](../cli/index.md) on your local computer.

You can either use
[Nexis Network's Install Tool](../cli/install.md#use-nexiss-install-tool) section from
...
Once the Nexis Network CLI is installed, you can return to this document once you are
...
```
nexis-cli 1.14.17 (src:b29a37cf; feat:3488713414)
```
...
Create a new Ubuntu user, named `nexis`, for running the validator:
...
Next, change the ownership of the directory to your `nexis` user:

```
sudo chown -R nexis:nexis /mnt/ledger
```
...
```
sudo chown -R nexis:nexis /mnt/accounts
```
...
```
scp validator-keypair.json nexis@<server.hostname>:
scp vote-account-keypair.json nexis@<server.hostname>:
```
...
## Switch to the nexis User

On the validator server, switch to the `nexis` user:

```
su - nexis
```

## Install The Nexis Network CLI on Remote Machine

Your remote machine will need the Nexis Network cli installed to run the validator
software. Refer again to
[Nexis Network's Install Tool](../cli/install.md#use-nexiss-install-tool) or
...
In your nexis home directory (e.g. `/home/nexis/`), create a folder called `bin`.
Inside that folder create a file called `validator.sh` and make it executable:

```
mkdir -p /home/nexis/bin
touch /home/nexis/bin/validator.sh
chmod +x /home/nexis/bin/validator.sh
```

Next, open the `validator.sh` file for editing:

```
nano /home/nexis/bin/validator.sh
```
...
exec nexis-validator \
    ...
    --only-known-rpc \
    --log /home/nexis/nexis-validator.log \
    --ledger /mnt/ledger \
    --rpc-port 8899 \
...
```
/home/nexis/bin/validator.sh
```

The script should execute the `nexis-validator` process. In a new terminal
window, shh into your server, then verify that the process is running:

```
ps aux | grep nexis-validator
```

You should see a line in the output that includes `nexis-validator` with all
...
the commands mentioned earlier to check gossip and Nexis Network validators:

```
tail -f /home/nexis/nexis-validator*.log
```
