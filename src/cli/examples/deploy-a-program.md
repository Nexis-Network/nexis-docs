---
title: Deploy a Nexis Network Program with the CLI
pagination_label: "Nexis Network CLI: Deploy a Program"
sidebar_label: Deploy a Program
---

Developers can deploy on-chain
[programs](https://nexis.network/docs/terminology#program) (often called smart
contracts elsewhere) with the Nexis Network tools.

To learn about developing and executing programs on Nexis Network, start with the
[intro to Nexis Network programs](https://nexis.network/docs/core/programs) and then dig
into the details of
[developing on-chain programs](https://nexis.network/docs/programs).

To deploy a program, use the Nexis Network tools to interact with the on-chain loader
to:
...
## Redeploy a Nexis Network program
...
```bash
nexis program extend <PROGRAM_ID> <ADDITIONAL_BYTES>
```
...
`nexis-keygen recover` and the following 12-word seed phrase:
==================================================================================
valley flat great hockey share token excess clever benefit traffic avocado athlete
==================================================================================
To resume a deploy, pass the recovered keypair as
the [BUFFER_SIGNER] to `nexis program deploy` or `nexis program write-buffer'.
...
```bash
nexis-keygen recover -o <KEYPAIR_PATH>
```
...
```bash
nexis program show --programs --buffer-authority <AUTHORITY_ADDRESS>
nexis program show --buffers --buffer-authority <AUTHORITY_ADDRESS>
```

To close a single account:

```bash
nexis program close <BADDRESS>
```

To close a single account and specify a different authority than the default:

```bash
nexis program close <ADDRESS> --buffer-authority <KEYPAIR_FILEPATH>
```

To close a single account and specify a different recipient than the default:

```bash
nexis program close <ADDRESS> --recipient <RECIPIENT_ADDRESS>
```
...
```bash
nexis program write-buffer <PROGRAM_FILEPATH> --buffer-authority <BUFFER_AUTHORITY_SIGNER>
```
...
`nexis program set-buffer-authority`:

```bash
nexis program set-buffer-authority <BUFFER_ADDRESS> --new-buffer-authority <NEW_BUFFER_AUTHORITY>
```
...
nexis program set-buffer-authority <BUFFER_PUBKEY> --new-buffer-authority <OFFLINE_SIGNER_PUBKEY>
```
...
nexis program upgrade <BUFFER_PUBKEY> <PROGRAM_ID> --sign-only --fee-payer <ONLINE_SIGNER_PUBKEY> --upgrade-authority <OFFLINE_SIGNER> --blockhash <VALUE>

# (5) (use online machine) use this signature to build and broadcast the upgrade transaction on-chain
nexis program upgrade <BUFFER_PUBKEY> <PROGRAM_ID> --fee-payer <ONLINE_SIGNER> --upgrade-authority <OFFLINE_SIGNER_PUBKEY> --blockhash <VALUE> --signer <OFFLINE_SIGNER_PUBKEY>:<OFFLINE_SIGNER_SIGNATURE>
```
Note:
