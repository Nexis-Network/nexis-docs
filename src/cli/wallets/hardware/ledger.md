---
title: Using Ledger Nano Hardware Wallets in the Nexis Network CLI
pagination_label: "Hardware Wallets in the Nexis Network CLI: Ledger Nano"
sidebar_label: Ledger Nano
---

This page describes how to use a Ledger Nano S, Nano S Plus, or Nano X to
interact with Nexis Network using the command line tools.

## Before You Begin

- [Set up a Nano with the Nexis Network App](https://support.ledger.com/hc/en-us/articles/360016265659-Nexis Network-SOL-?docs=true)
- [Install the Nexis Network command-line tools](../../install.md)

## Use Ledger Nano with Nexis Network CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the Nexis Network app on the Nano
...
```bash
nexis-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the Nexis Network CLI. The command returns your Ledger's unique
...
view any address, use the `nexis-keygen pubkey` command, as shown below,
...
nexis-keygen pubkey usb://ledger?key=1
nexis-keygen pubkey usb://ledger?key=2
```
...
addresses displayed by these commands are valid Nexis Network wallet addresses. The
...
`nexis balance` command:

```bash
nexis balance SOME_WALLET_ADDRESS
```
...
```bash
nexis balance 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri
```

You can also view the balance of any account address on the Accounts tab in the
[Explorer](https://explorer.nexis.network/accounts) and paste the address in the
...
non-existent accounts are treated the same in Nexis Network. This will change when your
...
the PIN, Ledger Live is not running, and the Nexis Network App is open on the device,
showing "Application is Ready".

The `nexis transfer` command is used to specify to which address to send
...
```bash
nexis transfer RECIPIENT_ADDRESS AMOUNT --keypair KEYPAIR_URL_OF_SENDER
```
...
```bash
~$ nexis-keygen pubkey usb://ledger?key=42
CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV

~$ nexis balance CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV
1.000005 SOL

~$ nexis transfer 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri 1 --keypair usb://ledger?key=42
...
finalized on the Nexis Network network. You can view details of this or any other
transaction by going to the Transaction tab in the
[Explorer](https://explorer.nexis.network/transactions) and paste in the
...
When the URL is not fully qualified, the Nexis Network CLI will prompt you with the
...
using the Nexis Network CLI `resolve-signer` command. For example, try connecting a
Nano to USB, unlock it with your pin, and running the following command:

```text
nexis resolve-signer usb://ledger?key=0/0
```
...
from the `resolve-signer` command anywhere a `nexis` command expects a
