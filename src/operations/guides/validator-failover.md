...
### Primary Validator
The following additional `nexis-validator` parameters are required to enable
...
following `nexis-validator` command-line argument changes:
...
```bash
$ nexis-validator wait-for-restart-window --identity validator-keypair.json \
  && nexis-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `nexis-validator set-identity
