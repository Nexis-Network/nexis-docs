...
First, setup Rust, Cargo and system packages as described in the Nexis Network [README](https://github.com/nexis-labs/nexis#1-install-rustc-cargo-and-rustfmt)
...
The demo code is sometimes broken between releases as we add new low-level features, so if this is your first time running the demo, you'll improve your odds of success if you check out the [latest release](https://github.com/nexis-labs/nexis/releases) before proceeding:
...
```bash
NDEBUG=1 ./multinode-demo/bench-tps.sh --entrypoint entrypoint.devnet.nexis.network:8001 --faucet api.devnet.nexis.network:9900 --duration 60 --tx_count 50
```

You can observe the effects of your client's transactions on our [metrics dashboard](https://metrics.nexis.network:3000/d/monitor/cluster-telemetry?var-testnet=devnet)
