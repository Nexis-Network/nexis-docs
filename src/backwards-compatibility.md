...
As the Nexis Network developer ecosystem grows, so does the need for clear expectations around
breaking API and behavior changes affecting applications and tooling built for Nexis Network.
In a perfect world, Nexis Network development could continue at a very fast pace without ever
...
- Nexis Network software releases include APIs, SDKs, and CLI tooling (with a few [exceptions](#exceptions)).
- Nexis Network software releases follow semantic versioning, more details below.
...
The Nexis Network RPC API, Rust SDK, CLI tooling, and SBF Program SDK are all updated and shipped
along with each Nexis Network software release and should always be compatible between `PATCH`
...
- `beta` software that runs on the Nexis Network Testnet cluster
- `stable` software that run on the Nexis Network Mainnet Beta and Devnet clusters
...
releases (e.g. 1.4.0) and are first run on Nexis Network's Testnet cluster. While running
...
- [`nexis-sdk`](https://docs.rs/nexis-sdk/) - Rust SDK for creating transactions and parsing account state
- [`nexis-program`](https://docs.rs/nexis-program/) - Rust SDK for writing programs
- [`nexis-client`](https://docs.rs/nexis-client/) - Rust client for connecting to RPC API
- [`nexis-cli-config`](https://docs.rs/nexis-cli-config/) - Rust client for managing Nexis Network CLI config files
- [`nexis-geyser-plugin-interface`](https://docs.rs/nexis-geyser-plugin-interface/) - Rust interface for developing Nexis Network Geyser plugins.
...
New Nexis Network runtime features are feature-switched and manually activated. Runtime features
...
Nexis Network provides publicly available RPC API nodes for all developers to use. The Nexis Network team
...
discourage dependence upon Nexis Network operated nodes.
...
The Web3.JS SDK also follows semantic versioning specifications but is shipped separately from Nexis Network
software releases.
