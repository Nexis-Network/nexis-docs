---
title: Staking SOL with the Nexis Network CLI
pagination_label: "Nexis Network CLI: Staking"
...
_stake account_. Nexis Network weights validator votes by the amount of stake delegated
to them, which gives those validators more influence in determining then next
valid block of transactions in the blockchain. Nexis Network then generates new SOL
periodically to reward stakers and validators. You earn more rewards the more
stake you delegate.

:::info
For an overview of staking, read first the
[Staking and Inflation FAQ](https://nexis.network/staking).
:::
...
the
[stake account address](https://nexis.network/docs/economics/staking/stake-accounts#account-address).
No need for a password or encryption here; this keypair will be discarded right
after creating the stake account.

```bash
nexis-keygen new --no-passphrase -o stake-account.json
```
...
[Stake and withdraw authorities](https://nexis.network/docs/economics/staking/stake-accounts#understanding-account-authorities)
can be set when creating an account via the `--stake-authority` and
`--withdraw-authority` options, or afterward with the `nexis stake-authorize`
...
accounts with the `nexis validators` command:
...
After delegating stake, use `nexis stake-account` to observe the changes to the
stake account:
...
existing stake account to a new one, use the `nexis split-stake` command:
