---
title: Nexis Network Validator Security Best Practices
...
## DO NOT Run The Nexis Network Validator as a Root User

It may be easier to get started by running your application as root, but it is a bad practice.

If there is an exploit in your system, a hacker could have full access if your Nexis Network application is running as the `root` user. Instead, see the [setup instructions](../setup-a-validator.md#nexis-user) for creating a user called `nexis` and running the application as the `nexis` user.
