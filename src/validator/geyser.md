---
title: Nexis Network Validator Geyser Plugins
...
- [`nexis-geyser-plugin-interface`] &mdash; This crate defines the plugin
interfaces.

- [`nexis-accountsdb-plugin-postgres`] &mdash; The crate for the referential
plugin implementation for the PostgreSQL database.

[`nexis-geyser-plugin-interface`]: https://docs.rs/nexis-geyser-plugin-interface
[`nexis-accountsdb-plugin-postgres`]: https://docs.rs/nexis-accountsdb-plugin-postgres
[`nexis-sdk`]: https://docs.rs/nexis-sdk
[`nexis-transaction-status`]: https://docs.rs/nexis-transaction-status

## The Plugin Interface

The Plugin interface is declared in [`nexis-geyser-plugin-interface`]. It
...
please refer to [`nexis-sdk`] and [`nexis-transaction-status`]
...
The [`nexis-accountsdb-plugin-postgres`] repository implements a plugin storing
...
```
{
	"libpath": "/nexis/target/release/libnexis_geyser_plugin_postgres.so",
...
The sample [postgresql.conf](https://github.com/nexis-network/nexis-network/blob/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/postgresql.conf)
...
Create the database. For example, the following creates a database named 'nexis':

```
sudo -u postgres createdb nexis -p 5433
```

Create the database user. For example, the following creates a regular user named 'nexis':

```
sudo -u postgres createuser -p 5433 nexis
```
...
```
psql -U nexis -p 5433 -h 10.138.0.9 -w -d nexis
```

#### Create the Schema Objects

Use the [create_schema.sql](https://github.com/nexis-network/nexis-network/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql)
to create the objects for storing accounts and slots.

Download the script from github:

```
wget https://raw.githubusercontent.com/nexis-network/nexis-network/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql
```

Then run the script:

```
psql -U nexis -p 5433 -h 10.138.0.9 -w -d nexis -f create_schema.sql
```
...
[drop_schema.sql](https://github.com/nexis-network/nexis-network/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/drop_schema.sql).
For example,

```
psql -U nexis -p 5433 -h 10.138.0.9 -w -d nexis -f drop_schema.sql
```
