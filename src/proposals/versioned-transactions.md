...
Messages transmitted to Nexis Network validators must not exceed the IPv6 MTU size to
ensure fast and reliable network transmission of cluster info over UDP.
Nexis Network's networking stack uses a conservative MTU size of 1280 bytes which,
after accounting for headers, leaves 1232 bytes for packet data like serialized
transactions.

Developers building applications on Nexis Network must design their on-chain program
...
deploying Nexis Network programs.
...
affect transaction processing in the Nexis Network program runtime beyond the
...
because it adds an extra step for transaction processing. Instead, Nexis Network
...
Nexis Network can provide a new on-chain program which allows "Big" transactions to be
