...
If there is an exploit in your system, a hacker could have full access if your Nexis Network application is running as the `root` user. Instead, see the [setup instructions](../setup-a-validator.md#sol-user) for creating a user called `sol` and running the application as the `sol` user.
