---
title: Off-Chain Message Signing with the Nexis Network CLI
pagination_label: "Nexis Network CLI: Off-Chain Message Signing"
sidebar_label: Off-Chain Message Signing
---

Off-chain message signing is a method of signing non-transaction messages with
a Nexis Network wallet. This feature can be used to authenticate users or provide
...
```bash
nexis sign-offchain-message <MESSAGE>
```
...
```bash
nexis sign-offchain-message -k <KEYPAIR> <MESSAGE>
```
...
```bash
nexis sign-offchain-message -k <KEYPAIR> --version <VERSION> <MESSAGE>
```
...
firmware and Nexis Network Ledger App version 1.3.0 or later. After Ledger is
unlocked and Nexis Network Ledger App is open, run:

```bash
nexis sign-offchain-message -k usb://ledger <MESSAGE>
```
...
enabled in Nexis Network Ledger App. Also, due to the lack of UTF-8 support in Ledger
...
```bash
nexis verify-offchain-signature <MESSAGE> <SIGNATURE>
```
...
```bash
nexis verify-offchain-signature --signer <PUBKEY> <MESSAGE> <SIGNATURE>
```
...
```bash
nexis verify-offchain-signature --version <VERSION> <MESSAGE> <SIGNATURE>
```
...
[proposal](https://github.com/nexis-labs/nexis/blob/master/docs/src/proposals/off-chain-message-signing.md).
